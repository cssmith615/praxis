...
export function activate(context: vscode.ExtensionContext): void {
  ...
  // Command: Validate
  context.subscriptions.push(
    vscode.commands.registerCommand('praxis.validate', () => {
  ...
  // Command: Run
  context.subscriptions.push(
    vscode.commands.registerCommand('praxis.run', () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showInformationMessage('No active editor.');
        return;
...
function getConfig(): PraxisConfig {
  const cfg = vscode.workspace.getConfiguration('praxis');
  return {
    executablePath: cfg.get<string>('executablePath', 'praxis'),
    validateOnSave: cfg.get<boolean>('validateOnSave', true),
    mode: cfg.get<string>('mode', 'dev'),
  };
}

// ── Validation ────────────────────────────────────────────────────────────────

function validateDocument(doc: vscode.TextDocument, showSuccess = false): void {
  const cfg = getConfig();
  const filePath = doc.fileName;

  const args = ['validate', filePath, '--mode', cfg.mode];

  cp.execFile(cfg.executablePath, args, { timeout: 15_000 }, (err, stdout, stderr) => {
    diagnosticCollection.delete(doc.uri);
    ...
    if (showSuccess || diagnostics.length > 0) {
      vscode.window.showWarningMessage(
        `Praxis: ${diagnostics.length} error(s). See Problems panel.`
      );
    }
  });
}
...
function runDocument(doc: vscode.TextDocument): void {
  ...
  terminal.show();
  terminal.sendText(
    `${cfg.executablePath} run --file "${filePath}" --mode ${cfg.mode}`
  );
}
...
function parseDiagnostics(
  ...
  for (const line of lines) {
    ...
    if (/^(validation (errors|failed)|parse error:?\s*$)/i.test(cleaned)) continue;
    ...
    const range = new vscode.Range(
      ...
      new vscode.Position(lineNum, doc.lineAt(Math.min(lineNum, doc.lineCount - 1)).text.length)
    );
  ...
  if (diagnostics.length === 0 && output.trim()) {
    const range = new vscode.Range(
      ...
      new vscode.Position(0, doc.lineAt(0).text.length || 1)
    );
